    ...
    for record_path in record_paths:
        ...
        try:
            ...
            for row in rows:
                ...
                record_relative = row[0].replace("\\", "/")
                if (
                    "__pycache__" in record_relative.split("/")
                    or record_relative.endswith((".pyc", ".pyo"))
                ):
                    # pip may append unhashed interpreter-generated bytecode rows to
                    # the installed RECORD. Bytecode is required to be absent, so
                    # these rows are not executable source-manifest members.
                    continue
